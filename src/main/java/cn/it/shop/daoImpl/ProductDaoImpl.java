...
public class ProductDaoImpl extends BaseDaoImpl<Product> implements ProductDao {
	...
	public Pager<Product> getSearchProduct(int pageNum, int pageSize, String name){
		...
		String hql ="FROM Product p WHERE p.commend=true AND p.open=true AND p.name LIKE :name ORDER BY p.date DESC";
		//计算总记录条数
		...
		productList = getSession().createQuery(hql)
				.setFirstResult(fromIndex).setMaxResults(pageSize)
						.setString("name", "%" +name + "%")
